...
export default function DeleteMemberConfirmModal({ isOpen, onClose, onConfirm, memberName }) {
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={null} maxWidth="max-w-sm">
      <div className="text-center">
        ...
        <h3 className={cn("text-xl font-serif font-bold mb-3", theme === 'light' ? "text-stone-900" : "text-stone-50")}>
          {t('dashboard.delete_modal.member_title') || 'Delete Member?'}
        </h3>
        
        <p className={cn("text-sm mb-8 leading-relaxed", theme === 'light' ? "text-stone-500" : "text-stone-400")}>
          <span className="font-bold">"{memberName}"</span><br/>
          {t('dashboard.delete_modal.member_description') || 'This member will be removed from the family. This action cannot be undone.'}
        </p>
        ...
        <div className="flex flex-col gap-3">
          ...
          >
            {t('dashboard.delete_modal.confirm_delete_member') || 'Yes, Delete Member'}
          </button>
          <button
            onClick={onClose}
            className={cn(
          ...
          >
            {t('dashboard.delete_modal.cancel') || 'Cancel'}
          </button>
        </div>
      </div>
    </Modal>
