...
export default function PublicFiltersBar({ filter, setFilter, sortBy, setSortBy, sortDirection, theme }) {
  const [mobileSortOpen, setMobileSortOpen] = useState(false);

  const sortOptions = useMemo(
    () => [
      { key: 'name', label: 'Name', labelAlt: 'Name' },
      { key: 'availability', label: 'Availability', labelAlt: 'Availability' },
      { key: 'expiryDate', label: 'Expiry Date', labelAlt: 'Expiry Date' },
    ],
    []
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2 md:gap-4">
        <span className={cn('text-xs uppercase tracking-widest mr-2', theme === 'light' ? 'text-stone-400' : 'text-stone-600')}>Sort</span>
